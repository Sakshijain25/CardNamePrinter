...
class Sender:
    ...
    def send(self, contact_name, pdf_file_path):
        ...
        WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, '_3YS_f _2A1R8')))

        # Find the contact with full name match
        contacts = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.XPATH, '//div[@class="_3YS_f _2A1R8"]')))

        # contacts = driver.find_elements_by_xpath('//span[@class="_3ko75"]')
        for contact in contacts:
            if contact.text == contact_name:
                contact.click()
